...
I𝒟 = κ(s₀) ∪ I𝒪
frames = [I𝒪, Iℛ, I𝒲, I𝒟]
∅, Γ1, Γ12, Γ123 = Set{Int}.([[],[1],[1,2], [1, 2, 3]])
...
@test normal_form(Ex, Σψ) == 
  C([], [:(ψ ⊢ 0)], [:(3ψ ⊢ 0), :(ψ ⊢ 2ψ), :(2ψ ⊢ ψ)], ∅)
...
@test normal_form(C([:(P(1) ⊢ Q(1,2))], [:(P(3) ⊢ Q(3,2))], refl, ∅), Σ) == 
  I𝒟nf
...
@test normal_form(κ(tsψ′), Σψ) == ⊤′
# ... whereas with `P` around `{ψ⁺, ψ⁺ψ⁻}` is neither `𝒲(ψ⁺)` nor `ℛ(ψ⁺)`:
# `ψ⁺P⁺P⁻ ∉`, though `ψ⁺ + Z⁺Z⁻ ∈` for `Z ⊆ {ψ}`
